...
Weight each response by its mean pairwise cosine similarity to all other responses.
"""
...
def scw_vote(embeddings, parsed_answers):
    """
    Semantic Consensus Weighting: weight each response by its mean
    pairwise cosine similarity to all other responses, then sum weights per answer.
    """
    n = len(embeddings)
    if n < 2:
    ...
    for i in range(n):
        if parsed_answers[i] is None:
            continue
        # Mean cosine similarity to all OTHER responses
...
def evaluate(json_path, embedder_name, dataset_name):
    """Run SCW evaluation on a generated results file."""
    ...
    scw_acc = correct_scw / total * 100

    print(f"\n{'='*50}")
    print(f"Dataset: {dataset_name} | Embedder: {embedder_name}")
    print(f"Majority Vote Accuracy: {maj_acc:.2f}%")
    print(f"SCW Accuracy:           {scw_acc:.2f}%")
    print(f"Improvement:            {scw_acc - maj_acc:+.2f}%")
    print(f"{'='*50}")

    return {"majority_vote": maj_acc, "scw": scw_acc}
...
def parse_args():
    parser = argparse.ArgumentParser(description="Semantic Consensus Weighting evaluation")
